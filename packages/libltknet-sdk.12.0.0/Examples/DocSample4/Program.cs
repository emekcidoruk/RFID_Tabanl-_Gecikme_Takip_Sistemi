    ...
    class Program
    {
        ...
        static void Reader_OnRoAccessReportReceived(MSG_RO_ACCESS_REPORT msg)
        {
            ...
            if (msg.TagReportData != null)
            {
                ...
                for (int i = 0; i < msg.TagReportData.Length; i++)
                {
                    ...
                    // collect some data for velocity calculations
                    ...
                    if (CalculateVelocity(out double velocity))
                    {
                        directionCount++;
                        ...
                        data += " Velocity: " + filteredVelocity.ToString("F2");
                    }
        ...
        static bool CalculateVelocity(out double velocity)
        {
                ...
                (lastReadTime < currentReadTime))
            {
                ...
                // if our phase change is close to 180 degrees, you can see we
        ...
        static void Usage()
        {
            Console.WriteLine("usage: docsample4.exe <readerName|IP>");
            return;
        }
        ...
        static void Main(string[] args)
        {
            ...
            #region ADDRoSpecWithXML
            {
                ...
                try
                {
                    FileStream fs = new(@"..\..\..\addRoSpec.xml", FileMode.Open);
                    StreamReader sr = new(fs);
                    string s = sr.ReadToEnd();
                    fs.Close();

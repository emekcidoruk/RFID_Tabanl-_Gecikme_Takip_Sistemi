    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                // Assign an event handler that will be called when unhandled error needs to be handled
                // Do required cleanup inside the handler.
                reader.ErrorNotification += OnErrorNotification;
        ...
        private static void OnErrorNotification(ImpinjReader reader, Exception error)
        {
            // This event handler is called when any unhandled errors occur during processing and needs to stop processing and
            // clean-up.
            Console.WriteLine("ErrorNotifcation received from {0} ({1})", reader.Name, reader.Address);
        }
